...
โดยอ้างอิงจาก requirement source ที่ระบุว่าระบบต้องรองรับผู้ใช้งานหลายระดับ
หลายฝ่าย และต้องควบคุมการเข้าถึงด้วย RBAC, audit trail และมาตรฐานด้านความปลอดภัย
...
จาก requirement source พบประเด็นสำคัญดังนี้:
...
เพื่อให้เอกสารเริ่มต้นไม่ซับซ้อนเกินไป รอบนี้กำหนดกลุ่มผู้ใช้งานหลัก 5 กลุ่ม:

```mermaid
flowchart LR
  subgraph Requirement["Requirement Groups"]
  ...
  HR -. "phase decision later" .-> Future
```

| Role        | ใช้แทนกลุ่มจาก requirement                           | จุดประสงค์หลัก                                                          |
| ----------- | ---------------------------------------------------- | ----------------------------------------------------------------------- |
...
- `Client` หรือ external customer ยังไม่ถูกนับเป็น primary internal role
  ในรอบนี้ เพราะต้องแยกเป็น client portal หรือ external access model ภายหลัง
...
- `HR` ยังไม่แยกเป็น role หลัก เพราะ HR/payroll ถูกจัดเป็นคำถามค้างใน scope
  และอาจอยู่คนละ phase
...
หลังจาก role baseline นี้นิ่งแล้ว หน้า [User Roles](/docs/roles/roles)
จะลงรายละเอียดหน้าที่ของแต่ละ role และหน้า
[Permissions](/docs/roles/permissions) จะกำหนด matrix ว่าแต่ละ role ดู เพิ่ม
แก้ไข อนุมัติ ส่งออก หรือลบข้อมูลใดได้บ้าง
