# Permissions

Access Profile เป็นชุดสิทธิ์เริ่มต้นที่ใช้ได้ทุกแผน ส่วนสิทธิ์รายบุคคลใช้ปรับ
ให้เหมาะกับหน้าที่ของสมาชิกแต่ละคน และเปิดใช้ตั้งแต่แผน PRO ขึ้นไป

การมีสิทธิ์ไม่ได้ทำให้แผนของ Workspace เปิดโมดูลเพิ่ม และการเปลี่ยนแผน
ไม่เปลี่ยนสิทธิ์ของสมาชิกโดยอัตโนมัติ

## Access Rules

สมาชิกใช้งานรายการหนึ่งได้เมื่อผ่านทุกเงื่อนไข:

1. บัญชีสมาชิกยังใช้งานได้
2. แผนของ Workspace เปิดโมดูลหรือฟีเจอร์นั้น
3. Access Profile หรือสิทธิ์รายบุคคลอนุญาตให้ดำเนินการ
4. ข้อมูลอยู่ใน Workspace ทีม แฟ้มงาน หรือขอบเขตที่ได้รับมอบหมาย
5. สถานะรายการและระดับอนุมัติรองรับการดำเนินการ

แผนที่สูงขึ้นไม่ให้สิทธิ์สมาชิกเพิ่มเติมโดยอัตโนมัติ และการมีสิทธิ์ไม่ได้เปิด
โมดูลที่แผนไม่รองรับ

## Permission Actions

- view: ดูข้อมูลหรือรายการ
- create: สร้างข้อมูลใหม่
- edit/update: แก้ไขข้อมูล
- delete/cancel: ลบหรือยกเลิกรายการตามกติกาของข้อมูล
- export: ส่งออกเอกสาร รายงาน หรือข้อมูล
- approve: อนุมัติรายการตามระดับและขอบเขตที่กำหนด
- assign: มอบหมายงานหรือเปลี่ยนผู้รับผิดชอบ
- manage: จัดการการตั้งค่า สมาชิก Access Profile หรือข้อมูลตั้งต้น

สิทธิ์หนึ่งรายการเขียนเป็นรหัสในรูปแบบ `resource.action` ซึ่งเป็นสิ่งที่ระบบ
ใช้ตรวจจริงตอนทำรายการ ตัวอย่างรหัสที่ระบบกำหนดไว้

```text
case.view
case.edit
case.delete

client.view

document.upload

billing.approve

role.manage
```

**รหัสสิทธิ์เป็นมาตรฐานกลางของทั้งระบบ ไม่ผูกกับ Workspace ใด** Workspace
เพิ่มรหัสใหม่เองไม่ได้ ทำได้เพียงเลือกว่า Access Profile ใดมีรหัสใดบ้าง

ผลที่ตามมาสองข้อ

- Business logic ต้องตรวจจากรหัสสิทธิ์ ไม่ใช่จากชื่อ Access Profile
  ดูตัวอย่างการเขียนที่ [Development Rules](/docs/development/rules)
- การเพิ่มความสามารถใหม่ให้ระบบคือการเพิ่มรหัสสิทธิ์ใหม่เข้า catalog
  ไม่ใช่การเพิ่ม Access Profile

## Starting Rules

- ใช้สิทธิ์เริ่มต้นจาก Access Profile ที่ตรงกับหน้าที่
- Workspace Admin เพิ่มหรือลดสิทธิ์เป็นรายบุคคลได้เมื่อใช้แผน PRO ขึ้นไป
  ส่วนแผน STANDARD และ ESSENTIALS ใช้ Access Profile อย่างเดียว
- ข้อมูลของแต่ละ Workspace ต้องแยกจากกัน
- การสร้างหรือแก้ไขรายการไม่รวมสิทธิ์อนุมัติโดยอัตโนมัติ
- การลบ ส่งออก อนุมัติ และเปลี่ยนสิทธิ์ต้องบันทึกประวัติ
- ตำแหน่งงานไม่ให้สิทธิ์จนกว่า Workspace Admin จะกำหนด
- ฟีเจอร์ที่เป็น TBD ยังไม่เปิดใช้งานแม้สมาชิกมีสิทธิ์

## Matrix Notation

ตารางด้านล่างใช้คำสั้น ๆ เพื่อให้อ่านง่าย:

| Notation | Meaning                                                               |
| :------: | --------------------------------------------------------------------- |
|  Config  | ตั้งค่าระบบหรือข้อมูลตั้งต้น ไม่ได้หมายถึงเห็นข้อมูลปฏิบัติการทั้งหมด |
|   Full   | ดู สร้าง แก้ไข ส่งออก และจัดการรายการในพื้นที่งานนั้น                 |
|   Work   | ทำได้เฉพาะแฟ้มงานที่รับผิดชอบ อยู่ในทีม หรือได้รับมอบหมาย             |
| Support  | ช่วยเตรียมและอัปเดตข้อมูล แต่ไม่อนุมัติขั้นสุดท้าย                    |
|   Read   | ดูข้อมูลหรือรายงานอย่างเดียว                                          |
|  Assign  | มอบหมายงาน เปลี่ยนผู้รับผิดชอบ หรือจัดลำดับงาน                        |
| Approve  | ตรวจสอบหรืออนุมัติรายการสำคัญ                                         |
|  Export  | ส่งออกข้อมูล เอกสาร หรือรายงาน                                        |
|    No    | ไม่มีสิทธิ์ตามปกติ                                                    |

## Permission Matrix

ทุกสิทธิ์ในตารางนี้อยู่ภายใน Workspace เดียวกันเท่านั้น และสิทธิ์ของ Lawyer กับ
Assistant ต้องถูกจำกัดด้วยแฟ้มงานกฎหมาย ทีม หรือรายการที่ได้รับมอบหมายเสมอ
ในตารางนี้ Client หมายถึงลูกความหรือผู้รับบริการ, Matter หมายถึงแฟ้มงานกฎหมาย
และ Document หมายถึงเอกสารทางกฎหมาย ตารางนี้แสดงขอบเขตสูงสุดของ Access Profile
ไม่รับประกันว่าแผนของ Workspace เปิดโมดูลนั้น

| <span style={{ display: 'block', textAlign: 'center' }}>Area</span> |       Admin       |      Lawyer      |      Assistant      |        Finance         |         HR          |       Manager       |
| :------------------------------------------------------------------ | :---------------: | :--------------: | :-----------------: | :--------------------: | :-----------------: | :-----------------: |
| Workspace & Member                                                  |   Config / Full   |        No        |         No          |           No           |         No          |  Read team members  |
| Master Data                                                         |   Config / Full   |       Read       |        Read         |  Config finance data   |   Config HR data    |        Read         |
| Client                                                              |   Support only    |       Work       |       Support       | Read billing contacts  |         No          |    Read overview    |
| Matter                                                              |   Support only    |   Work / Full    |       Support       |  Read billing status   |         No          |   Read / Approve    |
| Document & Template                                                 | Config templates  |       Work       |       Support       | Read finance documents |         No          |  Approve / Export   |
| Task & Calendar                                                     | Config categories |       Work       |   Work / Support    |   Work finance tasks   | Work leave calendar |    Read / Assign    |
| Quotation & Service Fee                                             |      Config       |    Work draft    |       Support       |          Full          |         No          |  Approve / Export   |
| Billing & Payment                                                   |      Config       | Read own matters | Read assigned items |          Full          |         No          |  Approve / Export   |
| Finance Foundation                                                  |      Config       |        No        |         No          |          Full          |         No          |   Read / Approve    |
| HR & Payroll                                                        |  Config settings  |        No        |         No          |           No           |        Full         |   Read / Approve    |
| Reports & Dashboard                                                 | Read system/audit |  Read own/team   | Read assigned work  |  Read finance reports  |   Read HR reports   |    Full overview    |
| Audit Log & Security                                                |       Full        |        No        |         No          |   Read finance audit   |    Read HR audit    | Read management log |

## Approval Matrix

รายการที่มีผลต่อเอกสารทางกฎหมาย การเงิน หรือสิทธิ์ผู้ใช้งานควรแยกคนเตรียมข้อมูล
ออกจากคนอนุมัติ เพื่อลดความผิดพลาดและตรวจสอบย้อนหลังได้ง่าย

| Item                     | Prepare / Edit                 | Approve / Confirm                                 | Audit Note                                                    |
| ------------------------ | ------------------------------ | ------------------------------------------------- | ------------------------------------------------------------- |
| Document final approval  | Lawyer, Assistant              | Manager หรือ Senior Lawyer ที่มีสิทธิ์เฉพาะขอบเขต | บันทึกผู้แก้ไข ผู้ตรวจ authority/scope และเวลาที่อนุมัติ      |
| Quotation approval       | Lawyer, Finance                | Manager                                           | บันทึกส่วนลด เงื่อนไขพิเศษ และเวอร์ชัน quotation              |
| Billing adjustment       | Finance                        | Manager                                           | บันทึกเหตุผลการแก้ invoice, payment หรือยอดค้าง               |
| Payment refund/write-off | Finance                        | Manager                                           | บันทึกจำนวนเงิน เหตุผล และผู้อนุมัติ                          |
| Leave request            | พนักงาน (self-service) หรือ HR | Manager หรือ alternate HR                         | policy/balance/overlap และ requester/approver แยกกัน          |
| Payroll run              | HR                             | Manager A1; privileged/org-wide เพิ่ม Admin A2    | maker/A1/A2/System post แยกกันและ Posted immutable            |
| Personnel lifecycle      | HR หรือ scoped Manager         | alternate/scoped Manager                          | Admin execute access; System transition; reconcile ทุก domain |
| Delete / restore data    | เจ้าของข้อมูลหรือผู้รับผิดชอบ  | Manager หรือ Admin ตามประเภทข้อมูล                | ใช้ soft delete สำหรับข้อมูลสำคัญ                             |
| Permission change        | Admin                          | Manager; privileged ใช้ Manager A1 + Admin A2     | บันทึก Access Profile และสิทธิ์ก่อนกับหลังการเปลี่ยนแปลง      |
| Sensitive export         | ผู้มีสิทธิ์ตาม module          | Manager เมื่อเป็นข้อมูลสำคัญ                      | บันทึกไฟล์ เวลา ผู้ส่งออก และขอบเขตข้อมูล                     |

## Data Boundary Rules

- Admin มีหน้าที่ตั้งค่าและช่วยแก้ปัญหาระบบ แต่ไม่ควรเห็นหรือแก้เนื้อหางาน
  กฎหมายโดยไม่จำเป็น
- Lawyer และ Assistant เห็นข้อมูลตามแฟ้มงานกฎหมาย ทีม หรือรายการที่ได้รับ
  มอบหมาย
- Finance เห็นข้อมูลที่จำเป็นต่อ quotation, billing, payment และ finance
  foundation แต่ไม่จำเป็นต้องเห็นรายละเอียดเอกสารทางกฎหมายทั้งหมด
- HR เห็นเฉพาะข้อมูลบุคลากร เงินเดือน และการลา ไม่เห็นข้อมูล Matter, document
  ทางกฎหมาย หรือ billing ของลูกค้า และข้อมูลเงินเดือนรายบุคคลไม่ควรให้ Finance
  เข้าถึงโดยตรง
- Manager เห็นภาพรวมและอนุมัติรายการสำคัญตามระดับความรับผิดชอบ ไม่ใช่เห็นทุก
  ข้อมูลของทุกทีมโดยอัตโนมัติ
- การลบ ส่งออก อนุมัติ เปลี่ยนสิทธิ์ และแก้ไขข้อมูลการเงินต้องมี audit log เสมอ
- ถ้ามีแฟ้มงานที่ต้องจำกัดเป็นพิเศษ ควรมีสิทธิ์ระดับแฟ้มงานกฎหมายเพิ่มเติมเหนือ
  Access Profile ปกติ
- Client Contact ใช้ Portal Membership และ publication scope ไม่ได้รับ internal
  role จากการมี Client record
- ตำแหน่ง เช่น Partner หรือ Owner ไม่ทำให้เห็นทุกข้อมูลจนกว่า Workspace Admin
  จะกำหนด Access Profile สิทธิ์รายบุคคล และขอบเขตที่เหมาะสม

Authority, scope, maker-checker และ toxic permission combination ให้ยึด
permission matrix และกฎการแยกหน้าที่ในหน้านี้เป็นหลัก

## Review Rule

ทบทวน matrix นี้ทุกไตรมาส รวมถึงเมื่อ Access Profile ระดับอนุมัติ
ขอบเขตโมดูล หรือสิทธิ์สำคัญเปลี่ยน

## Related Documents

- [Roles Overview](/docs/roles)
- [User Roles](/docs/roles/roles)
- [Role Mapping](/docs/roles/role-mapping)
- [Plans & Pricing](/docs/plans)
- [Feature Comparison](/docs/plans/feature-comparison)
- [Plan Rules](/docs/plans/plan-rules)
