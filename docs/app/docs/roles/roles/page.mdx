...
หน้านี้อธิบาย runtime role หลักของ Legal ERP Platform ในภาษาที่ทีมธุรกิจ
และทีมพัฒนาใช้ร่วมกันได้ Role เหล่านี้เป็น authorization baseline สำหรับ
permission และ workflow ไม่ใช่ชื่อตำแหน่งงานหรือ Pricing Plan
...
ที่ได้รับ Manager เพิ่มต้องผ่าน authority/SoD review แยกจาก Admin role

ตำแหน่งและหน้าที่ต้องผ่าน mapping ก่อนให้ runtime role หรือ permission bundle
และ Plan entitlement ไม่เพิ่มสิทธิ์ให้บัญชีผู้ใช้โดยอัตโนมัติ
...
Admin ใน baseline นี้หมายถึง Tenant Admin ผู้ดูแลการตั้งค่าและความปลอดภัยของ
องค์กรหนึ่ง Tenant เท่านั้น ผู้ดูแล platform ไม่ได้รับ role นี้ใน Tenant
และต้องใช้ service/support access ที่แยกจากบัญชีผู้ใช้งานปกติ
...
## Business User Group Boundary

ตารางนี้เป็น mapping direction สำหรับออกแบบ ไม่ใช่ automatic grant รายละเอียด
action, scope และ authority จะอยู่ใน Business User Group to Role Mapping
หลังผ่านการทบทวน

| Business User Group | Runtime Direction | Boundary |
| --- | --- | --- |
| Owner | Manager + approved oversight bundles | ไม่ได้ Finance/Admin หรือ cross-Tenant access อัตโนมัติ |
| Admin | Admin | เป็น system executor; ไม่เป็น business approver ของรายการเดียวกัน |
| Partner | Lawyer; เพิ่ม Manager เมื่อรับผิดชอบกำกับงาน | Partner title อย่างเดียวไม่ให้ approve ทุกประเภท |
| Managing Partner | Lawyer + candidate Manager authority | ห้ามอนุมาน A1/A2 จากชื่อตำแหน่ง ต้องกำหนด authority แยก |
| Senior Lawyer | Lawyer + scoped review/approve bundle | bundle จำกัด document/Matter/team/type และห้าม self-approve |
| Junior Lawyer | Lawyer | ไม่ได้รับ review/approve bundle อัตโนมัติ |
| Accounting | Finance | จำกัด legal content และห้ามเห็น individual payroll/personnel values |

กลุ่มผู้ใช้งานเพิ่มเติมที่กระบวนการทำงานต้องใช้:

| Workflow User Group | Runtime Direction | Boundary |
| --- | --- | --- |
...
| Client Contact | Portal Membership | external identity/scope ตาม SOP-CLI-002 ไม่ใช่ internal role |
| Platform Operator | Controlled support access | ไม่ใช้ Tenant Admin role และต้องมี expiry/audit |

## Approved Baseline Decisions

- Senior Lawyer ใช้ Lawyer + scoped review/approve permission ไม่สร้าง role
  หลักเพิ่มจนมีการอนุมัติให้เป็น runtime role
- Client Contact ใช้ Portal Membership ตาม SOP-CLI-002 และ controlled external
  sharing ตาม SOP-DOC-001
- ยังไม่สร้าง Sales role แยก โดย Lawyer/Finance จัดทำ quotation และ Manager
  อนุมัติ
- Admin ใน baseline หมายถึง Tenant Admin ส่วน platform support ใช้
  service/support access ที่แยกและอยู่ภายใต้ Emergency Access control
- Manager ถูกจำกัดด้วย Tenant, team, Matter และ responsibility scope
